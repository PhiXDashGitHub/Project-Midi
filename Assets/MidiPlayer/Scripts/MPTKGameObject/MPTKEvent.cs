        ...
        public override string ToString()
        {
            ...
            switch (Command)
            {
                case MPTKCommand.NoteOn:
                    result = string.Format("NoteOn\tTrk:{0:00} Ch:{1:00}\tTick:{2}\tNote:{3}\tDuration:{4,2}\tVelocity:{5}\n",
                      Track, Channel, Tick, Value, Math.Round(Duration, 2), Velocity);
                    break;
                case MPTKCommand.NoteOff:
                    result = string.Format("NoteOff\tTrk:{0:00} Ch:{1:00}\tTick:{2}\tNote:{3}\tDuration:{4,2}\tVelocity:{5}\n",
                      Track, Channel, Tick, Value, Math.Round(Duration, 2), Velocity);
                    break;
                ...
                case MPTKCommand.MetaEvent:
                    ...
                    break;
                default:
                    result = string.Format("Unknown cmd\tTrk:{0:00} Ch:{1:00}\tTick:{2}\tNote:{3}\tDuration:{4,2}\tVelocity:{5}\n",
                     Track, Channel, Tick, Value, Math.Round(Duration, 2), Velocity);
                    break;
            }
            return result;
